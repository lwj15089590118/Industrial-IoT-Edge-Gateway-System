...
| FR-04 | 时序存储 | 数据写入 InfluxDB `factory_metrics` 库，保留 30 天（降采样为预留扩展，见 §5.5） |
...
| NFR-04 | 存储容量 | 1 秒 1 帧 ≈ 每天 8.64 万点/测点；30 天保留策略控制磁盘（降采样连续查询为预留扩展，未启用，见 §5.5） |
| NFR-05 | 安全性（演示基线） | MQTT 匿名仅限内网；REST 启用基于环境变量的 CORS 白名单（默认仅放行本机前端）；metric/window 白名单校验防 InfluxQL 注入；后端默认仅监听 127.0.0.1（容器经 API_HOST 覆盖） |
...
系统在 Docker 内网中的实际部署拓扑如下，所有服务处于同一用户自定义网络，通过服务名互相寻址（DNS 由 Docker 内嵌提供），仅五个端口映射到宿主机，且默认全部绑定 `127.0.0.1` 回环地址（仅本机可访问；确需外部调试时自行改为 `0.0.0.0:` 前缀）：
...
磁盘占用对任何服务器都毫无压力；真正的压力在前端渲染——6 小时窗口含 21600 个原始点，Chart.js 逐点绘制会明显卡顿。解决方案是查询接口按时间范围自适应聚合窗口（15 分钟用 10s 窗口约 90 点、1 小时用 30s 约 120 点、6 小时用 5m 约 72 点），由 InfluxDB 在线聚合完成。InfluxDB 的 5 分钟降采样连续查询（`cq_metrics_5m` → `metrics_5m`）为预留能力，尚未接入查询链路——为避免"建而不用的死数据"，初始化脚本中暂不创建该 CQ，待查询端切换到降采样表时再恢复。
...
- REST API 增加访问鉴权（如 Token/JWT）——当前为无认证的演示基线，安全默认值仅依靠 127.0.0.1 绑定与 CORS 白名单兜底；
